...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Jefe}.
 */
@RestController
    ...
    public ResponseEntity<Jefe> partialUpdateJefe(@PathVariable(value = "id", required = false) final Long id, @RequestBody Jefe jefe)
        ...
        Optional<Jefe> result = jefeRepository
            ...
            .map(existingJefe -> {
                ...
                if (jefe.getApellidoJefe() != null) {
                    existingJefe.setApellidoJefe(jefe.getApellidoJefe());
                }
                if (jefe.getTelefonoJefe() != null) {
                    existingJefe.setTelefonoJefe(jefe.getTelefonoJefe());
                }
                if (jefe.getCorreoJefe() != null) {
                    existingJefe.setCorreoJefe(jefe.getCorreoJefe());
                }
